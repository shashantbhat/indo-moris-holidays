export default function Hero (){
    return(
        <div className="relative w-screen h-screen">
            <img
                src="/assets/background_pic.jpg"
                className="w-full h-full object-cover z-0"
                alt="Beautiful Destination"
            />
        
            {/* Overlay for slight dark effect */}
            {/* <div className="absolute inset-0 bg-black bg-opacity-40 z-10"/> */}

            {/* Hero Content */}
            <div className="absolute top-[10%] w-full flex items-center justify-center z-20 px-4">
                <div className="text-center">
                    <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-white leading-tight drop-shadow-lg">
                        Discover The <span className="text-blue-300">World</span>,<br className="hidden md:block" /> One Journey At A Time.
                    </h1>
                    <p className="mt-6 text-lg sm:text-xl md:text-2xl text-gray-200 font-medium drop-shadow-md">
                        Unforgettable destinations. Curated experiences. Memories that last forever.
                    </p>
                    <div className="mt-8">
                        <button className="px-3 py-1 text-lg  bg-black text-white transition drop-shadow-lg rounded-2xl">
                        Start Exploring
                        </button>
                    </div>
                </div>
            </div>
        </div>
    )
}